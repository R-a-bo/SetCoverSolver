...
parser.add_argument('definition', type=int,
...
parser.add_argument('--n_repeats', type=int, default=3, choices=[1, 2, 3, 4, 5],
...
def main():
    ...
    file_names = [elt for elt in os.listdir(path_read) if (
                str(definition) + ':1' in elt and regexp_p.search(elt) and regexp_q.search(elt) and elt.count(
            'p=') == 1 and elt.count('q=') == 1 and elt.split('_')[-1:][0][
                    0].isdigit())]  # make sure the right files are selected
